...
export default function DeleteEmployeeModal({
  ...
  return (
    <Modal
      ...
      label={`Delete ${name}`}
      onClose={onClose}
      description={`Removing ${name} from ${employee.department.name} changes more than this one record:`}
      primary={{
        label: submitting ? 'Deleting…' : `Delete ${name}`,
        variant: 'danger',
        disabled: !accepted || submitting,
        onClick: confirm,
      }}
      secondary={{ label: `Keep ${name}`, disabled: submitting }}
    >
